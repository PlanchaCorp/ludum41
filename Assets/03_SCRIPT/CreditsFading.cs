...
public class CreditsFading : MonoBehaviour {
	...
	void FixedUpdate ()
    {
        float firstCanvasOpacity = (Time.time - 0) / (firstCanvasApparitionDelay - 0);
        if (firstCanvasOpacity >= 0 && firstCanvasOpacity <= 1)
            firstCanvas.GetComponent<CanvasGroup>().alpha = firstCanvasOpacity;

        float firstCanvasOpacityWhenDisappearing = 1 - (Time.time - firstCanvasDisparitionDelay) / (secondCanvasApparitionDelay - firstCanvasDisparitionDelay);
        ...
        float secondCanvasOpacity = (Time.time - firstCanvasDisparitionDelay) / (secondCanvasApparitionDelay - firstCanvasDisparitionDelay);
        if (secondCanvasOpacity >= 0 && secondCanvasOpacity <= 1)
            secondCanvas.GetComponent<CanvasGroup>().alpha = secondCanvasOpacity;

        float secondCanvasOpacityWhenDisappearing = 1 - (Time.time - secondCanvasDisparitionDelay) / (thirdCanvasApparitionDelay - secondCanvasDisparitionDelay);
        ...
        float thirdCanvasOpacity = (Time.time - secondCanvasDisparitionDelay) / (thirdCanvasApparitionDelay - secondCanvasDisparitionDelay);
        if (thirdCanvasOpacity >= 0 && thirdCanvasOpacity <= 1)
            thirdCanvas.GetComponent<CanvasGroup>().alpha = thirdCanvasOpacity;

        float thirdCanvasOpacityWhenDisappearing = 1 - (Time.time - thirdCanvasDisparitionDelay) / (nextSceneDelay - thirdCanvasDisparitionDelay);
